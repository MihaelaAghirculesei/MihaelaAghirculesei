...
A CLI that scans an entire website for WCAG 2.1 AA violations and maps every finding to the EN 301 549 / BFSG clause it breaches – producing a citeable conformance report in JSON/HTML/PDF. Built as 28 individually reviewed pull requests; the key architecture decisions are captured in eight ADRs.
...
- **Supply chain:** published to npm with SLSA build provenance via OIDC trusted publishing – no long-lived tokens
